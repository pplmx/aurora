...
func NewVotingApp(storage oldvoting.Storage, repo domainvoting.Repository, service domainvoting.Service) *model {
...
func (m *model) handleRegisterCandidate() tea.Msg {
	...
	candidate := domainvoting.NewCandidate(name, party, program)
	if err := m.repo.SaveCandidate(candidate); err != nil {
		m.err = fmt.Sprintf("注册失败: %v", err)
		return nil
	}

	m.successMsg = fmt.Sprintf("候选人已注册: %s", candidate.Name)
	m.resultMsg = fmt.Sprintf("候选人ID: %s\n党派: %s", candidate.ID, candidate.Party)
	m.view = "result"
	return nil
}
...
func (m *model) handleRegisterVoter() tea.Msg {
	...
	voter := domainvoting.NewVoter(name)
	voter.PublicKey = base64.StdEncoding.EncodeToString(pub)

	if err := m.repo.SaveVoter(voter); err != nil {
		m.err = fmt.Sprintf("注册失败: %v", err)
		return nil
	}
...
func (m *model) handleCastVote() tea.Msg {
	...
	voter, err := m.repo.GetVoter(voterPK)
	...
	candidate, err := m.repo.GetCandidate(candidateID)
	...
	record, err := oldvoting.CastVote(pubBytes, candidateID, privBytes, chain)
	if err != nil {
...
func (m *model) loadResults() {
	candidates, err := m.repo.ListCandidates()
	if err != nil {
...
func RunVotingTUI(storage oldvoting.Storage, repo domainvoting.Repository, service domainvoting.Service) error {
	p := tea.NewProgram(NewVotingApp(storage, repo, service))
